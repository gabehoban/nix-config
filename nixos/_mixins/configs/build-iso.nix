...
#!${pkgs.stdenv.shell}
if [ -z $1 ]; then
  ${pkgs.coreutils-full}/bin/echo "Usage: build-iso <console|gnome>"
  exit 1
fi
...
if [ -e $HOME/nix-config ]; then
  ...
  pushd $HOME/nix-config 2>&1 > /dev/null
  ...
  ${pkgs.coreutils-full}/bin/mkdir -p $HOME/Builds/nixos-$1 2>/dev/null
  ${pkgs.coreutils-full}/bin/cp result/iso/$ISO $HOME/Builds/nixos-$1/nixos.iso
  ${pkgs.coreutils-full}/bin/chown $USER: $HOME/Builds/nixos-$1/nixos.iso
  ${pkgs.coreutils-full}/bin/chmod 644 $HOME/Builds/nixos-$1/nixos.iso
  popd 2>&1 > /dev/null
else
