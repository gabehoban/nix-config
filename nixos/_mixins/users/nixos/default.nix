...
if [ ! -d "$HOME/nix-config/.git" ]; then
  ${pkgs.git}/bin/git clone https://github.com/wimpysworld/nix-config.git "$HOME/nix-config"
fi
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  ${pkgs.rsync}/bin/rsync -a --delete "$HOME/" "/mnt/home/$TARGET_USER/"
  ...
  sudo nixos-enter --root /mnt --command "cd /home/$TARGET_USER/nix-config; env USER=$TARGET_USER HOME=/home/$TARGET_USER ${pkgs.home-manager}/bin/home-manager switch --flake \".#$TARGET_USER@$TARGET_HOST\""
  sudo nixos-enter --root /mnt --command "${pkgs.coreutils-full}/bin/chown -R $TARGET_USER:users /home/$TARGET_USER"
